...
from models.common import _extend_ts_graph_order

class RxnGraphExtension(object):
    def __init__(self, order=3, num_types=len(BOND_TYPES)):
        self.order = order
    ...
    def __call__(self, data: Data):
        ...
        batch = torch.zeros(N, dtype=torch.long, device=edge_index.device)
        edge_index, edge_type_r, edge_type_p = _extend_ts_graph_order(
            N, edge_index, edge_type, batch, order=self.order
        )
        data.edge_index_extend = edge_index
        data.edge_type_r = edge_type_r
        data.edge_type_p = edge_type_p
        return data
...
class AddHigherOrderEdges(object):
    ...
    def __call__(self, data: Data):
        ...
        data.is_bond = data.edge_type < self.num_types**2
...
class AddPlaceHolder(object):
    def __call__(self, data: Data):
        data.pos_gen = -1.0 * torch.ones_like(data.pos)
        data.d_gen = -1.0 * torch.ones_like(data.edge_length)
        data.d_recover = -1.0 * torch.ones_like(data.edge_length)
        return data
...
class AddEdgeName(object):
    ...
    def __call__(self, data: Data):
        ...
        for i in range(data.edge_index.size(1)):
            ...
            name = "%s_%s_%s_%d_%d" % (
                self.bonds[data.edge_type[i].item()]
                if data.edge_type[i].item() in self.bonds
                else "E" + str(data.edge_type[i].item()),
                tail_name,
                head_name,
                tail,
                head,
...
class AddAngleDihedral(object):
        ...
        for j in range(n_atoms):
            for k in range(n_atoms):
                for l in range(n_atoms):
                    if bond_mat[j, k].item() == 0 or bond_mat[k, l].item() == 0:
                        continue
    ...
    def iter_dihedral_quartet(bond_mat):
        ...
        for i in range(n_atoms):
            for j in range(n_atoms):
                if i >= j:
                    continue
                if bond_mat[i, j].item() == 0:
                    continue
                for k in range(n_atoms):
                    for l in range(n_atoms):
                        if (k in (i, j)) or (l in (i, j)):
                            continue
                        if bond_mat[k, i].item() == 0 or bond_mat[l, j].item() == 0:
                            continue
                        yield (k, i, j, l)
    ...
    def __call__(self, data: Data):
        ...
        if "is_bond" in data:
            bond_mat = (
                to_dense_adj(data.edge_index, edge_attr=data.is_bond).long().squeeze(0)
                > 0
            )
        else:
            bond_mat = (
                to_dense_adj(data.edge_index, edge_attr=data.edge_type)
                .long()
                .squeeze(0)
                > 0
            )
        ...
        data.dihedral_index = torch.LongTensor(
            list(self.iter_dihedral_quartet(bond_mat))
        ).t()
