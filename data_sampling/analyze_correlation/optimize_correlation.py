...
for i in indices:
    ...
    try:
        ...
        # elif error_type == "MorseError-RMSD":
            ...
            err = calcMorseErrorfromXYZ(
                ref_xyz, prb_xyz, norm=True, alpha=alpha, beta=beta, gamma=gamma
            )
            err = float(err)
...
print(
    f"{error_type} (ang): {sum(geom_err_list)/len(geom_err_list):0.3f} ({min(geom_err_list):0.3f} ~ {max(geom_err_list):0.3f})"
)
print(
    f"dE (kcal/mol): {sum(dE_list)/len(dE_list):0.3f} ({min(dE_list):0.3f} ~ {max(dE_list):0.3f})"
)
...
screening_type = "DMAE"
screening_thr = 0.2  # reference: TSDiff, COV threshold for Grambow
