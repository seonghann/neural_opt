...
if __name__ == "__main__":
    ...
    for i, filename in enumerate(filenames):
        ...
        rmsd = calc_RMSD2(atoms_0, atoms_t)
        dmae = calc_DMAE(pos_0, pos_t).item()
        q_norm = calc_q_norm(geodesic_solver, pos_0, pos_t, _atom_type).item()
        results["rmsd"].append(rmsd)
        results["dmae"].append(dmae)
        results["q_norm"].append(q_norm)

        # 2) noise distribution in Riemannian
        rmsd = calc_RMSD2(atoms_0, atoms_t2)
        dmae = calc_DMAE(pos_0, pos_t2).item()
        q_norm = calc_q_norm(geodesic_solver, pos_0, pos_t2, _atom_type).item()
