...
class EquiUpdate(nn.Module):
    ...
    def forward(self, x, vec, nodeframe):
        # x : (N_atoms, channels), vec : (N_atoms, 3, channels), nodeframe : (N_atoms, 3, 3)
        vec = self.vec_proj(vec)  # (N_atoms, 3, 2*channels)
        vec1, vec2 = torch.split(vec, self.hidden_channels, dim=-1)  # (N_atoms, 3, channels), (N_atoms, 3, channels)

        scalrization = torch.sum(vec1.unsqueeze(2) * nodeframe.unsqueeze(-1), dim=1)  # (N_atoms, 3, channels)

        if self.reflect_equiv:
            scalrization[:, 1, :] = torch.abs(scalrization[:, 1, :].clone())
        scalar = (self.lin3(torch.permute(scalrization, (0, 2, 1)))).squeeze(-1)  # (N_atoms, channels)

        vec_dot = (vec1 * vec2).sum(dim=1)  # (N_atoms, channels)
        vec_dot = vec_dot * self.inv_sqrt_h  # (N_atoms, channels)

        x_vec_h = self.xvec_proj(torch.cat([x, scalar], dim=-1))  # (N_atoms, 3*channels)
        xvec1, xvec2, xvec3 = torch.split(x_vec_h, self.hidden_channels, dim=-1)  # (N_atoms, channels), (N_atoms, channels), (N_atoms, channels)

        dx = xvec1 + xvec2 + vec_dot  # (N_atoms, channels)
        dx = dx * self.inv_sqrt_2  

        dvec = xvec3.unsqueeze(1) * vec2  # (N_atoms, 3, channels)
...
class LEFTNet(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # self.embedding = nn.Linear(in_hidden_channels, hidden_channels)  # Deprecated by Seonghwan
        # self.embedding_out = nn.Linear(hidden_channels, in_hidden_channels)  # Deprecated by Seonghwan
        ...
        if False: # self.legacy:  Deprecated by Seonghwan
    ...
    def forward(
        ...
        dist_T = dist_T * all_edge_masks.squeeze(-1)
        ...
        frame = torch.cat(
            ...
            dim=-1,
        )  # masked by all_edge_masks
        # frame is edge-wise axis, shape: (n_edge, 3, 3)
        # frame[i] is i-th edge's frame, which consists of 3 orthonormal vectors
        # frame[i, :, 0] is ij-th edge's diff vector
        # frame[i, :, 1] is ij-th edge's normal plane vector
        # frame[i, :, 2] is ij-th edge's the other orthonormal vector
        # Thus the second index is the vector dimension (R^3) index and the third index is the number of basis index.
        radial_emb = self.radial_emb(dist)
        radial_emb = radial_emb * all_edge_masks

        radial_emb_T = self.radial_emb(dist_T)
        radial_emb_T = radial_emb_T * all_edge_masks

        f = self.radial_lin(radial_emb)
        f_T = self.radial_lin(radial_emb_T)
        ...
        NE1 = self.s2v(s, coord_diff.unsqueeze(-1), edge_index, f)  # masked by all_edge_masks
        # s2v function
        # s: (node information) scalar with dim 196 (channel=196)
        # f: (edge-distance information) scalar with dim 196 (channel=196)
        # coord_diff: edge vector with dim 3
        # NE1: (n_node, 3, 196)
        # NE1 is vector quantity, which is E(3) equivariant
        # NE1[i] is i-th node's vectors in R^3 (the number of 196 vectors)
        scalrization1 = torch.sum(NE1[i].unsqueeze(2) * frame.unsqueeze(-1), dim=1)  # masked by all_edge_masks
        scalrization2 = torch.sum(NE1[j].unsqueeze(2) * frame.unsqueeze(-1), dim=1)  # masked by all_edge_masks
        # NE1[i].unsqueeze(2) -> (n_edge, 3 (3D), 1, 196 (channel))
        # frame.unsqueeze(-1) -> (n_edge, 3 (3D), 3 (basis), 1)
        # scalrization1 -> (n_edge, 3 (basis), 196 (channel))
        # project ecah vectors (196) to basis of each frame -> (n_edge, 3 (basis), 196 (channel))
        # the projected vectors (3, 196) is SE(3) invariant.

        # Deprecated by Seonghwan
        # if self.reflect_equiv:
        #     scalrization1[:, 1, :] = torch.abs(scalrization1[:, 1, :].clone())  # why?
        #     scalrization2[:, 1, :] = torch.abs(scalrization2[:, 1, :].clone())

        scalar3 = (
            self.lin3(torch.permute(scalrization1, (0, 2, 1)))  # Bias term could be added here, not masked by all_edge_masks
        ...
        # torch.permute(scalrization1, (0, 2, 1)) -> (n_edge, 196, 3)
        # self.lin3 act on frames' (with 196 channel) basis coordinate (3 dim), and output 1 dim scalar
        # torch.permute(scalrization1, (0, 2, 1))[:, :, 0] is the first basis coordinate,
        # which is the diff vector between ij-nodes.
        ...
        # # b: nearest neighbor node's position (n_atom, 3)
        ...
        # nodeframe is node-centered axis, shape: (n_atom, 3, 3)
        pos_prjt = torch.sum(pos_frame.unsqueeze(-1) * nodeframe, dim=1)
        # project origin to each node's frame, shape: (n_atom, 3)
        # ,which implies relative position of each node to origin in each node's frame.

        vec = torch.zeros(s.size(0), 3, s.size(1), device=s.device)  # (N_atom, 3, channels)
        ...
        for i in range(self.num_layers):
            ...
            # Intra-state geometric update
            dx, dvec = self.message_layers[i](
                s,  # Node vector, updated by excluding edges in all_edge_masks
                vec,
                edge_index,
                radial_emb,  # masked by all_edge_masks
                edgeweight,  # not-masked by all_edge_masks
                coord_diff,  # masked by all_edge_masks
                coord_cross,  # masked by all_edge_masks
            )
            s = s + dx
            vec = vec + dvec
            s = s * self.inv_sqrt_2
