...
for pair in "${param_pairs[@]}"; do
  ...
  cmd_sampling="python riemannian_data_sampling.py \
    ...
    --t0 0 \
    --t1 ${t1} \
    --save_xyz xyz_alpha${alpha}_beta${beta}_gamma0.0_svdtol_${svd_tol}_t${t1} \
    --save_csv alpha${alpha}_beta${beta}_gamma0.0_svdtol_${svd_tol}_t${t1}.sampling.csv"
  ...
  cmd_analyze="python analyze_distribution.py \
    --config_yaml riemannian_data_sampling.yaml \
    --save_csv alpha${alpha}_beta${beta}_gamma0.0_svdtol_${svd_tol}_t${t1}.analyzing.csv \
    --xyz_path xyz_alpha${alpha}_beta${beta}_gamma0.0_svdtol_${svd_tol}_t${t1} \
    --t0_x 1 \
    --t1_x 1500 \
